...
def overlay_train_args(args: argparse.Namespace, cli_keys: set[str]) -> argparse.Namespace:
    ...
    never_overlay = {
        "checkpoint", "external_prob_path", "original_prob_path", "output_dir", "mode", "eval_batch_size",
        "query_modes", "delta_scales", "ensemble_alphas", "ensemble_query_modes",
        "ensemble_delta_scales", "decoder_prob_sources", "decoder_prob_mix_alphas",
        "skip_legacy_query_sources", "auprc_mode", "threshold_step", "compute_sample_fmax",
...
def run_eval(args: argparse.Namespace) -> Dict[str, Any]:
        ...
        for batch_i, batch in enumerate(tqdm(loader, desc="detr-diagnostic-eval")):
            ...
            with autocast(device_type=device.type, dtype=torch.bfloat16, enabled=amp_enabled):
                ...
                if orig_prob is not None:
                    pred_chunks["original_only"].append(orig_prob.detach().cpu().numpy().astype(store_dtype))
    ...
    if "external_only" in pred_map and ensemble_alphas:
        ...
        for k in query_keys:
            ...
            qmode = parts[1] if len(parts) > 1 else ""
    ...
    if bool(args.do_rare_analysis):
        ...
        if not rare_keys:
            # Keep rare analysis compact by default.
            rare_keys = [
                "backbone_base",
                "external_only",
...
def print_compact_summary(result: Dict[str, Any]) -> None:
    ...
    for key in sorted(result["metrics"].keys()):
        if key.startswith("prob_mix::") or key.startswith("query::decoderprob::") or key.startswith("ensemble_ext_alpha_") or \
        key.startswith("anchorlogit::decoderprob::") or key.startswith("ensemble_ext_alpha_"):
            _print_metric_row(key)
