...
class EvalWeakMSAGOWithQueryDecoder(nn.Module):
    ...
    @torch.no_grad()
    def build_topk_idx(self, base_logits: torch.Tensor) -> torch.Tensor:
        k = min(self.topk, base_logits.shape[1])
        score = torch.sigmoid(base_logits.float())
        return torch.topk(score, k=k, dim=1).indices

    def forward(self, x: torch.Tensor, permute_dims=(0, 3, 2, 1)):
        out = self.backbone(
            x,
        ...
        topk_idx = self.build_topk_idx(base_logits.detach())
