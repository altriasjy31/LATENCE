...
# Increment this whenever the go.norm parser changes.  It is logged at startup
# so users can verify that the intended script, rather than an older copy, is
# actually being executed.
PARSER_VERSION = "go.norm-role-colon-v4-artifact"
BUILD_ID = "boxsqel-go-only-20260725-v4"
...
def parse_simplified_axiom(line: str) -> Optional[List[Tuple[str, Tuple[str, ...]]]]:
    """Parse the compact normal-form syntax used by ``go.norm``.

    Supported class axioms::
    ...
    Supported role axioms (colon optional)::
    ...
    The role patterns are parsed with regular expressions *before* the generic
    token rules.  This deliberately handles ``SubPropertyOf:`` as one token and
    also tolerates arbitrary spaces around the colon.
    """
    line = line.strip()

    # go.norm role-chain syntax, e.g.
    #   BFO_0000066 o BFO_0000050 SubPropertyOf: BFO_0000066
    role_chain_match = re.fullmatch(
        r"(\S+)\s+(?:o|compose|chain)\s+(\S+)\s+"
        r"(?:SubPropertyOf|SubObjectPropertyOf)\s*:?\s*(\S+)",
        line,
    )
    if role_chain_match is not None:
        return [("role_chain", role_chain_match.groups())]

    # go.norm role-inclusion syntax, e.g.
    #   RO_0002213 SubPropertyOf: RO_0002211
    role_inclusion_match = re.fullmatch(
        r"(\S+)\s+(?:SubPropertyOf|SubObjectPropertyOf)\s*:?\s*(\S+)",
        line,
    )
    if role_inclusion_match is not None:
        return [("role_inclusion", role_inclusion_match.groups())]

    tokens = line.split()

    def keyword(index: int) -> str:
        return tokens[index].rstrip(":")

    if len(tokens) == 3 and keyword(1) == "SubClassOf":
        return [("nf1", (tokens[0], tokens[2]))]

    if len(tokens) == 5 and keyword(1) == "and" and keyword(3) == "SubClassOf":
        if tokens[4] == "owl:Nothing":
            return [("disjoint", (tokens[0], tokens[2]))]
        return [("nf2", (tokens[0], tokens[2], tokens[4]))]

    if len(tokens) == 5 and keyword(1) == "SubClassOf" and keyword(3) == "some":
        return [("nf3", (tokens[0], tokens[2], tokens[4]))]

    if len(tokens) == 5 and keyword(1) == "some" and keyword(3) == "SubClassOf":
...
def validate_go_norm_parser() -> None:
    """Fail fast if the parser cannot read the role syntax used by go.norm."""
    cases = {
        "BFO_0000066 o BFO_0000050 SubPropertyOf: BFO_0000066": [
            ("role_chain", ("BFO_0000066", "BFO_0000050", "BFO_0000066"))
        ],
        "RO_0002213 SubPropertyOf: RO_0002211": [
            ("role_inclusion", ("RO_0002213", "RO_0002211"))
        ],
        # Also test optional whitespace around the colon.
        "RO_0002212 SubPropertyOf : RO_0002211": [
            ("role_inclusion", ("RO_0002212", "RO_0002211"))
        ],
    }
    failures = []
    for text, expected in cases.items():
        actual = parse_simplified_axiom(text)
        if actual != expected:
            failures.append(f"{text!r}: expected {expected!r}, got {actual!r}")
    if failures:
        raise RuntimeError(
            "Internal go.norm parser self-test failed:\n  - " + "\n  - ".join(failures)
        )
...
def main(
    ...
    validate_go_norm_parser()
    LOGGER.info("go.norm parser self-test: passed")
