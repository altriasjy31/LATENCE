...
def import_elbox_model():
    """
    mOWL version compatibility:

    Newer versions may expose ELBE.
    Older examples/docs may expose ELBoxEmbeddings.
    """
    try:
...
@ck.option("--out-classes-file", "-ocf", default="go_elbox_classes.pkl",
           help="Output pickle file for class box embeddings.")
@ck.option("--out-relations-file", "-orf", default="go_elbox_relations.pkl",
           help="Output pickle file for object property embeddings.")
@ck.option("--embedding-size", "-es", default=50, type=int,
           help="Box embedding dimension.")
...
@ck.option("--margin", "-m", default=0.0, type=float,
           help="Margin. Accepted by some ELBox/ELBE versions.")
@ck.option("--reg-norm", "-rn", default=1.0, type=float,
           help="Reg norm if accepted by the installed mOWL version.")
