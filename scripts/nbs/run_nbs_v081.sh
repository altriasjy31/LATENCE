...
case "$VARIANT" in baseline|loss|graph) ;; *) echo 'Variant must be baseline, loss or graph' >&2; exit 2;; esac
PYTHON_BIN="${PYTHON_BIN:-python}"
CONFIG="${CONFIG:-nbs_models/nbs_protein_go/configs/bp_full_task_v0.8.1_${VARIANT}.json}"
WORK_ROOT="${WORK_ROOT:-outputs/latence_nbs_experiments/v081}"
...
REFERENCES_READY=0
RUNNER="scripts/nbs/train_nbs_full_task_v081.py"
...
case "$ACTION" in
  ...
  series)
    read -r -a checkpoint_steps <<< "${CHECKPOINT_STEPS:-100 300 600}"
    ...
    ;;
  *) echo 'Usage: bash scripts/nbs/run_nbs_v081.sh {prepare|references|smoke|pilot|train|evaluate|series} {baseline|loss|graph}' >&2; exit 2 ;;
esac
