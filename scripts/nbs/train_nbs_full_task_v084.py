#!/usr/bin/env python3
"""v0.8.4: multi-source sampled relation SAGE with full-GO discrete supervision."""
...
def training_objective(model, batch, variant, loss_config, support_config):
    ...
    if not torch.equal(batch["targets"], batch["positive_mask"].float()):
        raise ValueError("v084 supervision must be binary membership, without modelout probabilities")
    ...
    if "core_support_mass" in output:
        parts["difficult_gold_core_coverage"] = ((output["core_support_mass"].detach() > 0) & difficult).sum().float() / difficult.sum().clamp_min(1)
...
def prediction_implementation(variant):
    ...
    result["train_nbs_full_task_v084.py"] = sha256(Path(__file__))
    return result
...
def train(args, config, data, device, rank, world):
    ...
    if loss_config.graph_weight != 0:
        raise ValueError("v084 removes the old own-feature graph auxiliary: set loss.graph_weight=0")
    ...
    if args.resume:
        ...
        if saved.get("runner_version") != "0.8.4":
            raise ValueError("v0.8.4 needs a fresh run; earlier model architectures are incompatible")
        if saved.get("training_implementation") != training_implementation(variant):
            raise ValueError("resume requires unchanged v0.8.4 training implementation")
    ...
    for step in range(start + 1, total + 1):
        ...
        if step in points:
            ...
            if world > 1:
                ...
                all_rng[0] = rng
            if rank == 0:
                state = {"version": "0.8.4", "runner_version": "0.8.4", "variant": variant,
...
def evaluate(args, config, data, device):
    ...
    if saved.get("runner_version") != "0.8.4":
        raise ValueError("v0.8.4 evaluator requires a v0.8.4 checkpoint; use the original runner for old runs")
    ...
    if any(trained_implementation.get(key) != value for key, value in implementation.items()):
        raise ValueError("prediction implementation differs from the code that trained this v084 checkpoint")
    ...
    try:
        ...
        raise
    manifest = {"version": "0.8.4", "runner_version": "0.8.4", "variant": variant, "step": saved["step"],
    ...
    command = [sys.executable, str(ROOT / "scripts/nbs/eval_nbs_full_task_v084.py"),
...
def main():
    ...
    p.add_argument("--config", type=Path, default=ROOT / "nbs_models/nbs_protein_go/configs/bp_full_task_v0.8.4_fixed.json")
    p.add_argument("--work-dir", type=Path, default=ROOT / "outputs/latence_nbs_experiments/v084/fixed")
