...
cd -- "$PROJECT_ROOT"
ACTION="${1:-pilot}"
VARIANT="${2:-${VARIANT:-dual}}"
case "$VARIANT" in teacher|classifier|dual) ;; *) echo 'Variant must be teacher, classifier or dual' >&2; exit 2;; esac
PYTHON_BIN="${PYTHON_BIN:-python}"
CONFIG="${CONFIG:-nbs_models/nbs_protein_go/configs/bp_full_task_v0.8.2_${VARIANT}.json}"
WORK_ROOT="${WORK_ROOT:-outputs/latence_nbs_experiments/v082}"
...
ABLATIONS="${ABLATIONS:-${ABLATION:-full}}"
if [[ "$VARIANT" == dual ]]; then
  BRANCHES="${BRANCHES:-final classification}"
else
  BRANCHES="${BRANCHES:-final}"
fi
...
REFERENCES_READY=0
RUNNER="scripts/nbs/train_nbs_full_task_v082.py"
...
run_evaluation() {
  ...
  local branches=() ablations=() branch ablation
  read -r -a branches <<< "$BRANCHES"
  read -r -a ablations <<< "$ABLATIONS"
  for ablation in "${ablations[@]}"; do
    case "$ablation" in full|weak_off|core_off) ;; *) echo "Unknown ablation: $ablation" >&2; return 2;; esac
    for branch in "${branches[@]}"; do
      case "$branch" in final|classification) ;; *) echo "Unknown output branch: $branch" >&2; return 2;; esac
      if [[ "$VARIANT" == teacher && "$branch" == classification ]]; then
        echo 'The teacher variant has no classification branch.' >&2; return 2
      fi
      "$PYTHON_BIN" "$RUNNER" --stage evaluate "${COMMON[@]}" --checkpoint "$checkpoint" \
        --input-dir "$INPUT_DIR" --metadata-file "$METADATA_FILE" --ablation "$ablation" --branch "$branch" \
...
case "$ACTION" in
  ...
  series)
    ...
    for step in "${checkpoint_steps[@]}"; do
      ...
      [[ -f "$WORK_DIR/nbs_step${step}.pt" ]] || { echo "Checkpoint not found: $WORK_DIR/nbs_step${step}.pt (set CHECKPOINT_STEPS to the steps already trained)" >&2; exit 2; }
    done
    for step in "${checkpoint_steps[@]}"; do
      run_evaluation "$WORK_DIR/nbs_step${step}.pt"
    done
    ;;
  *) echo 'Usage: bash scripts/nbs/run_nbs_v082.sh {prepare|references|smoke|pilot|train|evaluate|series} {teacher|classifier|dual}' >&2; exit 2 ;;
esac
