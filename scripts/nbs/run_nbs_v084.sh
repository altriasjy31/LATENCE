...
cd -- "$PROJECT_ROOT"
ACTION="${1:-pilot}"
VARIANT="${2:-${VARIANT:-dynamic}}"
case "$VARIANT" in fixed|dynamic) ;; *) echo 'Variant must be fixed or dynamic' >&2; exit 2;; esac
PYTHON_BIN="${PYTHON_BIN:-python}"
CONFIG="${CONFIG:-nbs_models/nbs_protein_go/configs/bp_full_task_v0.8.4_${VARIANT}.json}"
WORK_ROOT="${WORK_ROOT:-outputs/latence_nbs_experiments/v084}"
...
REFERENCES_READY=0
RUNNER="scripts/nbs/train_nbs_full_task_v084.py"
...
case "$ACTION" in
  ...
  diagnose)
    ...
    "$PYTHON_BIN" scripts/nbs/eval_nbs_full_task_v084.py \
  ...
  series)
    ...
    ;;
  *) echo 'Usage: bash scripts/nbs/run_nbs_v084.sh {prepare|references|smoke|pilot|train|evaluate|series|diagnose} {fixed|dynamic}' >&2; exit 2 ;;
esac
