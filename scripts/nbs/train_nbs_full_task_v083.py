#!/usr/bin/env python3
"""v0.8.3: native graph context and exact core-GO messages with discrete supervision."""
...
from nbs_pg.full_task_data_v083 import FullTaskDataV083 as FullTaskData
...
def build_model(feature_dim, ontology, model_options, variant, device):
    if variant not in ("graph", "local", "no_graph"):
        raise ValueError("full_task.variant must be graph, local or no_graph")
    cfg = FullTaskModelConfigV083(**model_options)
    model = FullTaskGraphModelV083(feature_dim, ontology, cfg).to(device)
...
def forward_flags(variant, ablation="full"):
    return {
        "use_weak_go": variant != "no_graph" and ablation not in ("weak_off", "graph_off"),
        "use_core_go": variant != "no_graph" and ablation not in ("core_off", "graph_off"),
        "use_pp_context": variant == "graph" and ablation not in ("pp_off", "core_off", "graph_off"),
...
def training_objective(model, batch, variant, loss_config, support_config):
    ...
    # Hold the loss policy constant in trained graph/local/no_graph controls.
    ...
    if not torch.equal(batch["targets"], batch["positive_mask"].float()):
        raise ValueError("v083 supervision must be binary membership, without modelout probabilities")
...
def prediction_implementation(variant):
    names = ["full_task_model.py", "full_task_model_v081.py", "full_task_model_v083.py",
             "full_task_data.py", "full_task_data_v083.py", "local_loader.py"]
    folder = ROOT / "nbs_models/nbs_protein_go/nbs_pg"
    result = {name: sha256(folder / name) for name in names}
    result["train_nbs_full_task_v083.py"] = sha256(Path(__file__))
    return result
...
def training_implementation(variant):
    ...
    folder = ROOT / "nbs_models/nbs_protein_go/nbs_pg"
    for name in ("full_task_loss_v081.py", "full_task_evidence_v081.py"):
...
def train(args, config, data, device, rank, world):
    ...
    variant = cfg.get("variant", "graph")
    if bool(cfg.get("use_pp_context", True)) != (variant == "graph"):
        raise ValueError("graph uses native PP context; local/no_graph require use_pp_context=false")
    ...
    if loss_config.graph_weight != 0:
        raise ValueError("v083 removes the old own-feature graph auxiliary: set loss.graph_weight=0")
    ...
    if args.resume:
        ...
        if saved.get("runner_version") != "0.8.3":
            raise ValueError("v0.8.3 needs a fresh run; v081/v082 graph and supervision objectives are incompatible")
        if saved.get("training_implementation") != training_implementation(variant):
            raise ValueError("resume requires unchanged v0.8.3 training implementation")
        ...
        if old_cfg.get("variant", "graph") != variant or old_cfg.get("structural_support", {}) != cfg.get("structural_support", {}):
    ...
    for step in range(start + 1, total + 1):
        ...
        if step in points:
            ...
            if world > 1:
                ...
                all_rng[0] = rng
            if rank == 0:
                state = {"version": "0.8.3", "runner_version": "0.8.3", "variant": variant,
...
def evaluate(args, config, data, device):
    ...
    if saved.get("runner_version") != "0.8.3":
        raise ValueError("v0.8.3 evaluator requires a v0.8.3 checkpoint; use the original runner for old runs")
    ...
    variant = saved.get("variant", saved["config"]["full_task"].get("variant", "graph"))
    ...
    try:
        ...
        raise
    manifest = {"version": "0.8.3", "runner_version": "0.8.3", "variant": variant, "step": saved["step"],
                "branch": "final", "checkpoint_path": str(args.checkpoint.resolve()),
                ...
                "graph_forward": "shared_target_readout_with_native_core_anchor_context",
    ...
    command = [sys.executable, str(ROOT / "scripts/nbs/eval_nbs_full_task_v083.py"),
...
def main():
    ...
    p.add_argument("--config", type=Path, default=ROOT / "nbs_models/nbs_protein_go/configs/bp_full_task_v0.8.3_graph.json")
    p.add_argument("--work-dir", type=Path, default=ROOT / "outputs/latence_nbs_experiments/v083/graph")
