...
cd -- "$PROJECT_ROOT"
ACTION="${1:-pilot}"
PRESET="${2:-${PRESET:-schedule}}"
case "$PRESET" in constant|schedule|lowlr|aligned|dynamic) ;; *) echo 'Preset must be constant, schedule, lowlr, aligned or dynamic' >&2; exit 2;; esac
PYTHON_BIN="${PYTHON_BIN:-python}"
CONFIG="${CONFIG:-nbs_models/nbs_protein_go/configs/bp_full_task_v0.8.5_${PRESET}.json}"
WORK_ROOT="${WORK_ROOT:-outputs/latence_nbs_experiments/v085}"
...
REFERENCES_READY=0
RUNNER="scripts/nbs/train_nbs_full_task_v085.py"
...
case "$ACTION" in
  recompute)
    ...
    audit_args=(--recompute-comparison "$COMPARISON_PATH" --metadata-file "$METADATA_FILE"
      --output-dir "${AUDIT_OUTPUT_DIR:-$(dirname -- "$COMPARISON_PATH")/metric_audit_v085}")
    if [[ -n "${PATH_MAP:-}" ]]; then audit_args+=(--path-map "$PATH_MAP"); fi
    "$PYTHON_BIN" scripts/nbs/eval_nbs_full_task_v085.py "${audit_args[@]}"
    ;;
  ...
  diagnose)
    ...
    "$PYTHON_BIN" scripts/nbs/eval_nbs_full_task_v085.py \
  ...
  series)
    ...
    ;;
  *) echo 'Usage: bash scripts/nbs/run_nbs_v085.sh {recompute|prepare|references|smoke|pilot|train|evaluate|series|diagnose} {constant|schedule|lowlr|aligned|dynamic}' >&2; exit 2 ;;
esac
