#!/usr/bin/env python3
"""v0.8.6: matched shallow-encoder controls over unchanged v084 data, v081 loss and v085 schedule."""
...
from nbs_pg.local_loader_v086 import build_latence_nbs_stores
...
def build_model(feature_dim, ontology, model_options, variant, device):
    ...
    cfg = FullTaskModelConfigV086(**model_options)
    model = FullTaskGraphModelV086(feature_dim, ontology, cfg).to(device)
...
def training_objective(model, batch, variant, loss_config, support_config):
    ...
    if not torch.equal(batch["targets"], batch["positive_mask"].float()):
        raise ValueError("v086 supervision must be binary membership, without modelout probabilities")
...
def prediction_implementation(variant):
    names = ["full_task_model.py", "full_task_model_v081.py", "full_task_model_v083.py", "full_task_model_v084.py", "full_task_model_v086.py",
             "full_task_data.py", "full_task_data_v083.py", "full_task_data_v084.py", "local_loader.py", "local_loader_v086.py", "latence_graph_stores.py"]
    folder = ROOT / "nbs_models/nbs_protein_go/nbs_pg"
    result = {name: sha256(folder / name) for name in names}
    result["train_nbs_full_task_v086.py"] = sha256(Path(__file__))
    return result
...
def train(args, config, data, device, rank, world):
    ...
    if loss_config.graph_weight != 0:
        raise ValueError("v086 keeps the old own-feature graph auxiliary disabled: set loss.graph_weight=0")
    ...
    if args.resume:
        ...
        if saved.get("runner_version") != "0.8.6":
            raise ValueError("v0.8.6 needs a fresh run; older checkpoints lack this runner schedule/development contract")
        if saved.get("training_implementation") != training_implementation(variant):
            raise ValueError("resume requires unchanged v0.8.6 training implementation")
    ...
    if rank == 0:
        config["runtime"] = {"requested_steps": total, "world_size": world,
                             ...
                             "model_architecture_version": "0.8.6",
                             "data_architecture_version": "0.8.4",
                             "encoder_variant": model_config.encoder_variant,
                             "encoder_control": "matched_encoder_ablation; compare model/sampler/optimization contracts",
    ...
    points = {int(v) for v in cfg.get("checkpoints", [600, 1200, 2000]) if 0 < int(v) <= total} | {total}
    ...
    for step in range(start + 1, total + 1):
        ...
        if step in points:
            ...
            if world > 1:
                ...
                all_rng[0] = rng
            if rank == 0:
                state = {"version": "0.8.6", "runner_version": "0.8.6", "variant": variant,
                         ...
                         "model_architecture_version": "0.8.6", "data_architecture_version": "0.8.4",
                         "encoder_variant": model_config.encoder_variant,
...
def evaluate(args, config, data, device):
    ...
    if saved.get("runner_version") != "0.8.6":
        raise ValueError("v0.8.6 evaluator requires a v0.8.6 checkpoint; use the original runner for old runs")
    ...
    if any(trained_implementation.get(key) != value for key, value in implementation.items()):
        raise ValueError("prediction implementation differs from the code that trained this v086 checkpoint")
    requested_model_config = asdict(FullTaskModelConfigV086(**config["full_task"].get("model", {})))
    ...
    try:
        ...
        raise
    manifest = {"version": "0.8.6", "runner_version": "0.8.6", "variant": variant, "step": saved["step"],
                "branch": "final", "model_architecture_version": "0.8.6",
                "data_architecture_version": "0.8.4",
    ...
    command = [sys.executable, str(ROOT / "scripts/nbs/eval_nbs_full_task_v086.py"),
...
def main():
    ...
    p.add_argument("--config", type=Path, default=ROOT / "nbs_models/nbs_protein_go/configs/bp_full_task_v0.8.6_legacy.json")
    p.add_argument("--work-dir", type=Path, default=ROOT / "outputs/latence_nbs_experiments/v086/legacy")
    ...
    # All prepare/train/evaluate/dev paths share the explicit v086 provenance loader.
