#!/usr/bin/env python3
"""v0.8.2: dense teacher, full classification and selected graph matching."""
...
from nbs_pg.full_task_data_v082 import FullTaskDataV082 as FullTaskData
...
def build_model(feature_dim, ontology, model_options, variant, device):
    if variant == "teacher":
        cfg = FullTaskModelConfigV081(**model_options)
        return FullTaskGraphModelV081(feature_dim, ontology, cfg).to(device), cfg
    if variant not in ("classifier", "dual"):
        raise ValueError("full_task.variant must be teacher, classifier or dual")
    cfg = FullTaskModelConfigV082(**model_options)
    return FullTaskGraphModelV082(feature_dim, ontology, cfg, variant=variant).to(device), cfg
...
def training_objective(model, batch, variant, loss_config, support_config):
    ...
    loss, parts = full_task_loss_v082(
        logits, batch["base_logits"], batch["targets"], batch["positive_mask"],
        batch["is_weak"], loss_config, teacher_prob=batch["teacher_prob"],
        teacher_available=batch["teacher_available"], structural_support=support,
        pu_exclusion_mask=excluded,
        classification_logits=output.get("classification_logits") if variant == "dual" else None,
        selected_mask=output.get("selected_mask") if variant == "dual" else None,
    )
...
def prediction_implementation(variant):
    names = ["full_task_model.py", "full_task_data.py", "full_task_data_v082.py",
             "full_task_model_v081.py"]
    if variant != "teacher":
        names.append("full_task_model_v082.py")
    folder = ROOT / "nbs_models/nbs_protein_go/nbs_pg"
    result = {name: sha256(folder / name) for name in names}
    result["train_nbs_full_task_v082.py"] = sha256(Path(__file__))
    return result
...
def training_implementation(variant):
    ...
    folder = ROOT / "nbs_models/nbs_protein_go/nbs_pg"
    for name in ("full_task_loss_v082.py", "full_task_evidence_v081.py"):
...
class ProteinCycle:
    ...
    def __init__(self, ids, seed):
        ...
        if not len(self.ids):
            raise ValueError("training role has no eligible proteins")
...
def validate(model, data, device, batch_size, *, ablations=False):
    ...
    for start in range(0, len(data.validation_ids), batch_size):
        ...
        for name, (weak_on, core_on) in modes.items():
            detail = model(batch, go_encoding=go, use_weak_go=weak_on, use_core_go=core_on, return_details=True)
            outputs[name].append(detail["logits"].sigmoid().cpu().numpy())
            if name == "full" and "classification_logits" in detail:
                classification.append(detail["classification_logits"].sigmoid().cpu().numpy())
    ...
    if classification:
        metrics["classification"] = metric_pack(y, np.concatenate(classification))
...
def train(args, config, data, device, rank, world):
    ...
    if args.resume:
        ...
        if saved.get("runner_version") != "0.8.2":
            raise ValueError("v0.8.2 needs a fresh run; v0.8.1 optimizer/model objectives are incompatible")
        if saved.get("training_implementation") != training_implementation(variant):
            raise ValueError("resume requires unchanged v0.8.2 training implementation")
        ...
        if old_cfg.get("variant", "dual") != variant or old_cfg.get("structural_support", {}) != cfg.get("structural_support", {}):
    ...
    for step in range(start + 1, total + 1):
        ...
        if step % cfg.get("log_every", 25) == 0 or step in points:
            ...
            if rank == 0:
                ...
                print(f"[step {step}/{total}] loss={row['loss']:.5f} base={row['base_objective']:.5f} "
                      ...
                      f"gain={row['objective_gain']:+.5f} refine={row.get('contrib_refine', 0):.5f} "
        ...
        if step in points:
            ...
            if world > 1:
                ...
                all_rng[0] = rng
            if rank == 0:
                state = {"version": "0.8.2", "runner_version": "0.8.2", "variant": variant,
...
def evaluate(args, config, data, device):
    ...
    if saved.get("runner_version") != "0.8.2":
        raise ValueError("v0.8.2 evaluator requires a v0.8.2 checkpoint; use v0.8.1 for old runs")
    ...
    variant = saved.get("variant", saved["config"]["full_task"].get("variant", "dual"))
    branch = getattr(args, "branch", "final")
    if branch == "classification" and variant == "teacher":
        raise ValueError("teacher control has no independent classification branch")
    ...
    if branch == "classification":
        destination = destination / "classification"
    ...
    if probability_path.exists():
        ...
        reuse = (recorded.get("checkpoint_sha256") == sha256(args.checkpoint)
                 ...
                 and recorded.get("branch") == branch and recorded.get("variant") == variant)
        if not reuse:
    ...
    try:
        ...
        for start in range(0, 0 if reuse else rows, size):
            ...
            output = model(batch, go_encoding=go, use_weak_go=args.ablation != "weak_off",
                           use_core_go=args.ablation != "core_off", return_details=True)
            logits = output["classification_logits"] if branch == "classification" else output["logits"]
        ...
        raise
    manifest = {"version": "0.8.2", "runner_version": "0.8.2", "variant": variant, "step": saved["step"],
                "branch": branch, "checkpoint_path": str(args.checkpoint.resolve()),
    ...
    command = [sys.executable, str(ROOT / "scripts/nbs/eval_nbs_full_task_v082.py"),
...
def main():
    ...
    p.add_argument("--config", type=Path, default=ROOT / "nbs_models/nbs_protein_go/configs/bp_full_task_v0.8.2_dual.json")
    p.add_argument("--work-dir", type=Path, default=ROOT / "outputs/latence_nbs_experiments/v082/dual")
    ...
    p.add_argument("--branch", choices=("final", "classification"), default="final")
    ...
    if args.stage == "prepare":
        # Validate the large training teacher before launching GPU workers.
        teacher = data.data_contract()["teacher_v082"]
        print(f"[teacher] source={teacher['probability_path']} shape={teacher['shape']} "
              f"dtype={teacher['dtype']} sha256={teacher['probability_sha256']}", flush=True)
