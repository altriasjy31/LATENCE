# NBS v0.5.6 completed-training evaluation plan (inductive-r4)
...
the checkpoint, input arrays, and CUDA memory. r4 ships the four files as one
contract and checks API version 4 before Stage-1 input preparation. The model
change adds no checkpoint parameters: the trained `similar_to` layers are
reused for isolated `core -> test` messages, so an existing epoch-150
checkpoint remains strictly loadable.

r4 also separates metric meanings:
...
Extract the r4 patch at the project root, preserving paths. The first launcher
line expected after environment validation is:

```
[NBS inference API preflight] version=4 status=ok
```

The export process repeats the check before CUDA and prints:

```
[NBS inference API] version=4 status=ok
```
...
1. both API checks report version 4;
