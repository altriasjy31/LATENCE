...
from scripts.nbs import train_nbs_full_task_v081 as runner
...
def _config(data):
    ...
    config["full_task"]["variant"] = "graph"
    config["full_task"]["model"].update(candidate_dropout=.15, activation_checkpointing=True, attention_backend="math")
    config["full_task"]["loss"] = asdict(FullTaskLossConfigV081(hard_pu_k=1, background_pu_k=1, ranking_hard_k=1, ranking_random_k=1))
    return config
...
def test_resume_refuses_changed_baseline_clipping(tmp_path):
    data = _data(tmp_path / "data")
    config = _config(data)
    _train(data, config, tmp_path / "run", 2)
...
def test_baseline_arm_reproduces_v080_updates_and_loss_arm_runs(tmp_path):
    from test_full_task_runner import _config as baseline_config, _train as old_train
    data = _data(tmp_path / "data")
    original = baseline_config(data)
    old = old_train(data, copy.deepcopy(original), tmp_path / "old", 2)
    baseline = copy.deepcopy(original)
    baseline['full_task']['variant'] = 'baseline'
    new = _train(data, baseline, tmp_path / "baseline", 2)
    for key in old['model']:
        assert torch.equal(old['model'][key],new['model'][key]), key
    controlled = copy.deepcopy(baseline)
    controlled['full_task']['variant'] = 'loss'
    controlled['full_task']['loss'] = asdict(FullTaskLossConfigV081(
        graph_weight=0, hard_pu_k=1, background_pu_k=1, ranking_hard_k=1, ranking_random_k=1))
    corrected = _train(data,controlled,tmp_path / 'loss',2)
    assert corrected['history'][-1]['contrib_ranking'] > 0
    assert corrected['history'][-1]['contrib_graph_aux'] == 0
...
def test_real_inductive_export_and_existing_metric_cli(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(runner.FullTaskGraphModelV081, "forward", forbidden_reprediction)
    runner.evaluate(args, config, data, torch.device("cpu"))
...
def test_two_rank_training_saves_synchronized_checkpoint(tmp_path):
    ...
    assert torch.count_nonzero(saved["model"]["correction_decoder.2.weight"]) > 0
