...
module = load("v084_eval", ROOT / "scripts/nbs/eval_nbs_full_task_v084.py")
...
def arguments(path, branch="final"):
    ...
    manifest = {
        "version": "0.8.4", "runner_version": "0.8.4", "variant": "dynamic",
...
def test_stale_v081_manifest_rejected(files):
    ...
    with pytest.raises(ValueError, match="not a v0.8.4 output"):
        module.main(argv)
...
def intervention_results(tmp_path):
    ...
    for ablation in module.ABLATIONS:
        item = {"evaluation_version": "0.8.4", "task": "bp", "num_samples": 1800,
...
def test_prior_variant_names_rejected(files, variant):
    ...
    with pytest.raises(ValueError, match="unknown v0.8.4 variant"):
        module.main(argv)
