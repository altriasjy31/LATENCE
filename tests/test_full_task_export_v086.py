...
from scripts.nbs import eval_nbs_full_task_v086 as evaluator
from test_full_task_data_v080 import inference_fixture
from test_full_task_runner_v086 import _data, _config, _train
...
def test_real_inductive_export_metrics_and_verified_prediction_reuse(tmp_path, monkeypatch):
    ...
    for mode in ("full", "pp_off"):
        ...
        assert comparison["evaluation_version"] == "0.8.6"
        ...
        assert provenance["manifest"]["runner_version"] == "0.8.6"
        assert provenance["manifest"]["model_architecture_version"] == "0.8.6"
        assert provenance["manifest"]["data_architecture_version"] == "0.8.4"
        assert provenance["manifest"]["encoder_variant"] == "preln"
        assert provenance["manifest"]["model_config"]["pp_edge_dropout"] == .1
    ...
    with monkeypatch.context() as patch:
        patch.setattr(runner.FullTaskGraphModelV086, "forward", forbidden_reprediction)
    ...
    with monkeypatch.context() as patch:
        ...
        assert json.loads((audit_dir / "metric_audit_v086.json").read_text()) == audit
        assert (audit_dir / "metric_audit_v086.tsv").is_file()
